...
filter out list of todos whenever our application re-renders.  
In this situation, we could use `Provider` to do the filter for us.

For that, assume that our application have an existing [StateNotifierProvider]
...
that listens to it unless the value changed.
...
This change both improved the performances of our button and had the interesting
